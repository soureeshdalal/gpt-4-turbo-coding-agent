...
def create_agents() -> tuple[Agent, Agent, Agent]:
    ...
    coding_agent = Agent(
        model=OpenAIChat(
            id="gpt-4-turbo",  # Change this to a valid model
...
def initialize_sandbox() -> None:
    try:
        if st.session_state.sandbox:
            st.session_state.sandbox.close()  # Close existing sandbox instance

        os.environ['E2B_API_KEY'] = st.session_state.e2b_key
        st.session_state.sandbox = Sandbox(timeout=60)

        # ❗ Instead of `sandbox.files.clear()`, reinitialize the sandbox itself
        st.session_state.sandbox = Sandbox(timeout=60)
...
def execute_code_with_agent(execution_agent: Agent, code: str, sandbox: Sandbox) -> str:
    try:
        ...
        if execution.error:
            ...
            error_prompt = f"Execution error encountered:\n{execution.error}"
            response = execution_agent.run(error_prompt)
            return f"⚠️ Execution Error:\n{response.content}"

        # **Fix: List only user-generated files (not system files)**
        try:
            files = [file for file in sandbox.files.list("/") if not file.name.startswith('.')]
        except:
            files = []
...
def main() -> None:
    st.title("GPOT-4-turbo Coding Agent")
